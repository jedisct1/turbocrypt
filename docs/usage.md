...
`--password` encrypts the key file itself.
TurboCrypt detects protected keys when it loads them and prompts for the password automatically:
...
You can also add protection to a plain key, change an existing password, or remove the password later.
...
As a result, decryption requires both the same key file and the exact same context.
Omitting the context or supplying another string fails authentication.
...
Contexts provide separate encryption domains while reusing a key file.
If the context is meant to remain secret, do not put it in shell history or scripts.
...
TurboCrypt first writes a temporary file and then renames it over the original, but that does not substitute for a backup.
...
However, the directory structure, file sizes and number of entries remain visible.

## Excluding files

`--exclude` may be repeated.
For example, this skips logs and the repository metadata:
...
Use `--dry-run` to check paths and exclusions before starting the real job.
It prints the file count and total size without modifying anything:
...
By default, verification authenticates the header and contents without writing the plaintext.

In contrast, quick verification authenticates only the header.
That is enough to check the key and context, but it says nothing about the integrity of the file contents.
...
`list` reports paths, encrypted sizes and a total without decrypting file contents.
When the names were encrypted, add the key and `--encrypted-filenames` to make them readable in the listing.
...
Keys, thread counts, buffer sizes, exclusions, symlink handling and filename encryption can be saved in the configuration:
...
See [Configuration](configuration.md) for storage locations and the full precedence rules.
